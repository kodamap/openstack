...
yum -y -q install openstack-glance python-glance python-glanceclient
...
openstack-config --set ${CONF} database connection mysql://glance:${GLANCE_DBPASS}@${controller}/glance
...
# In the [DEFAULT] section, configure the noop notification driver to disable notifications
# because they only pertain to the optional Telemetry service:
openstack-config --set ${CONF} DEFAULT notification_driver noop
