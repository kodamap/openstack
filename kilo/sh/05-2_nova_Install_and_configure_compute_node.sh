...
yum -y -q install openstack-nova-compute sysfsutils
...
openstack-config --set ${CONF} DEFAULT vnc_enabled True
openstack-config --set ${CONF} DEFAULT vncserver_listen 0.0.0.0
openstack-config --set ${CONF} DEFAULT vncserver_proxyclient_address ${compute}
openstack-config --set ${CONF} DEFAULT novncproxy_base_url http://${controller}:6080/vnc_auto.html
