...
install_components () {
  ...
  openstack-config --set ${CONF} database connection mysql://heat:${HEAT_DBPASS}@${controller}/heat
  ...
  openstack-config --set ${CONF} trustee user_domain_id default

  openstack-config --set ${CONF} clients_keystone auth_uri http://controller:5000
  openstack-config --set ${CONF} ec2authtoken auth_uri http://controller:5000/v3

  # In the [DEFAULT] section, configure the metadata and wait condition URLs:
  openstack-config --set ${CONF} DEFAULT heat_metadata_server_url http://${controller}:8000
  openstack-config --set ${CONF} DEFAULT heat_waitcondition_server_url http://${controller}:8000/v1/waitcondition
