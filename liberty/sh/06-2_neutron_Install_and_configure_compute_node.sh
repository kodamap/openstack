...
if [ $# -ne 3 ]; then
    echo "** Usage: $0 <controller ip> <OVERLAY_INTERFACE_IP_ADDRESS> <PUBLIC_INTERFACE_NAME>"
    exit 1
fi

controller=$1
OVERLAY_INTERFACE_IP_ADDRESS=$2
PUBLIC_INTERFACE_NAME=$3
...
# In the environment , the public network is named as "provider"
openstack-config --set ${CONF} linux_bridge physical_interface_mappings provider:${PUBLIC_INTERFACE_NAME}
...
openstack-config --set ${CONF} agent prevent_arp_spoofing True
...
openstack-config --set ${CONF} neutron auth_plugin password
openstack-config --set ${CONF} neutron project_domain_id default
openstack-config --set ${CONF} neutron user_domain_id default
...
if [ $# -ne 4 ]; then
    echo "** Usage: $0 <controller node ip> <compute ip> <OVERLAY_INTERFACE_IP_ADDRESS> <PUBLIC_INTERFACE_NAME>"
    exit 1
fi
...
ssh root@${compute} "/root/${SH} ${controller} ${OVERLAY_INTERFACE_IP_ADDRESS} ${PUBLIC_INTERFACE_NAME}"
