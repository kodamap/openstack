...
# ref. https://docs.openstack.org/ocata/networking-guide/deploy-ovs-selfservice.html
...
configure_openvswitch_agent () {
  ...
  openstack-config --set ${CONF} agent vxlan_udp_port 4789
  openstack-config --set ${CONF} agent l2_population False
  openstack-config --set ${CONF} agent drop_flows_on_start False

  # In the [ovs] section
  openstack-config --set ${CONF} ovs integration_bridge br-int
  openstack-config --set ${CONF} ovs tunnel_bridge br-tun
  openstack-config --set ${CONF} ovs local_ip ${OVERLAY_INTERFACE_IP_ADDRESS}
...
configure_l3_agent () {
  ...
  openstack-config --set ${CONF} DEFAULT external_network_bridge
  openstack-config --set ${CONF} DEFAULT agent_mode legacy
